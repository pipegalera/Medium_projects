# Background. LTSM and GRU in a nutsheell in 3 minutes.

The project aim is to predict stocks using only historical stock data. Tradicional Neural Networks are static, in the sense that they do not use loop the previous neuron values into the next ones. To solve this dynamism problem,  I will use recurrent neural networks (RNN). RNNs can be thought of as multiple copies of the same network, each passing the nueron value to a successor.

But not that fast. RNNs are no capable of handling “long-term dependencies.” If the present value depend a lot of the long run previous value, RNN tend to not being able to process this long term dependency chain. They do not do a great job predicting long run time series. With the task of predicting stocks, the  gap between the relevant information and the point where it is needed is very whide. How we take into account long-term dependencies? With a LSTM and GRU models. 
