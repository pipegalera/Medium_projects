# @Author: pipegalera
# @Date:   2020-04-16T23:04:55+02:00
# @Last modified by:   pipegalera
# @Last modified time: 2020-04-17T14:32:34+02:00
...
# Load data
grouped_shots_df = pd.read_csv('raw_data/league_shots_by_dist.csv')

# Shots per distance
grouped_shots_df.head()
fig = px.scatter(grouped_shots_df, x = 'avg_dist', y = 'shots_accuracy', size = 'shots_counts', color = 'shot_type')
fig.show()

# Shot chart

# Load coordinates data (X,Y)
with open('raw_data/league_hexbin_stats.pickle', 'rb') as f:
    league_hexbin_stats = pickle.load(f)

# From dict to dataframe
hexbin_data = pd.DataFrame()
for i in league_hexbin_stats.keys():
    hexbin_data['{}'.format(i)] = league_hexbin_stats['{}'.format(i)]
...
# Function to show stats when you point the hexagon
...
fig.add_trace(go.Scatter(
    x= hexbin_data.xlocs, y= hexbin_data.ylocs, mode='markers', name='markers',
    marker=dict(
        size= hexbin_data.freq_by_hex, sizemode='area', sizeref=2. * max(freq_by_hex) / (11. ** 2), sizemin=2.5,
        color= hexbin_data.accs_by_hex, colorscale=colorscale,
        colorbar=dict(
            thickness=15,
            x=0.84,
            y=0.87,
...
teamname = 'BOS'
with open('raw_data/' + teamname.lower() + '_hexbin_stats.pickle', 'rb') as f:
...
fig.update_layout(
    ...
    annotations=[
        go.layout.Annotation(
            ...
            y=0.05,
            showarrow=False,
            text="Thanks for the guide to @_jphwang",
            xref="paper",
            yref="paper"
        ),
    ],
